...
class Controleur{
    public static function lireObjets(){
        ...
        foreach($obj as $cle=>$valeur){
            array_push($ligne,static::$txt . $valeur->get(static::$key) . static::$link . static::$read. $valeur->get(static::$key).'>détails</a>'.static::$link . static::$delete. $valeur->get(static::$key).'>Delete</a>');
        }
