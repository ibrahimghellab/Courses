...
int rechercheDicho (const vector<int> & v, int val)
{
    ...
    bool trouve=false;
    int debut=0,fin=v.size()-1,milieu,indice=-1;
...
int recherchePetit (const vector<int> & tab, int val)
{
    ...
    bool premiertrouver=false,trouve=false;


    int debut=0,fin=tab.size()-1,milieu,indice=-1;
    while ((!trouve||!premiertrouver) && debut<=fin)
    {
        milieu=(debut+fin)/2;
