...
{
    int numPlace,nbVal=_lesPieces.size(),pos;
    bool premierecase=false;
    ...
    for(int i=0; i<nbVal; i++)
    {
        pos=i;
        premierecase=pos==0;
        while(premierecase==false && _lesPieces[pos].surface()<_lesPieces[pos-1].surface()  )
        {
            Piece p=_lesPieces[pos];
            _lesPieces[pos]=_lesPieces[pos-1];
            _lesPieces[pos-1]=p;
            pos--;
            premier=p==0;
        }
